...
export const getTags = asyncHandler(async (req, res) => {
    ...
    async function fetchData() {
        try {
            ...
            console.error(
                "Error fetching data from Lightcast API:",
                error.response?.data
            );
        }
    }
...
async function getAccessToken() {
    const data = qs.stringify({
        ...
        scopes: process.env.EMSI_SCOPES,
    });
        ...
        const user = await User.findOneAndUpdate(
            { kindeAuthId: kindeAuthId },
            {
                $push: {
                    ...
                    certifications: {
                        $each: dataToBeUpdated.certifications || [],
                    },
...
export const getPotentialUsers = asyncHandler(async (req, res) => {
    ...
    if (!userDescriptionOfTeam) {
        return res
            .status(400)
            .json({ error: "Description of team is required." });
    }
    ...
    try {
        ...
        try {
            ...
            return res
                .status(400)
                .json({ error: "Failed to parse user skills" });
        }
        ...
        const userQuery = `
             MATCH (currentUser:User {kindeAuthId: $kindeAuthId}) 
MATCH (u:User)-[r:HAS_SKILL]->(s:Skill)
WHERE ANY(skill IN $userSkillNames WHERE TOLOWER(s.name) CONTAINS TOLOWER(skill))
AND u.kindeAuthId <> $kindeAuthId  // Exclude the current user
AND NOT (u)-[:FRIEND]-(currentUser) 
WITH u, COLLECT(s.name) AS matchedSkills, 
...
RETURN u, matchedSkills, numSkills, totalScore
        ...
        const usersResult = await tx.run(userQuery, { userSkillNames, kindeAuthId });
