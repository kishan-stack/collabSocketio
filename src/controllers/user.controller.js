...
export const registerUser = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        if (userExists.records.length > 0) {
            return res
                ...
                .json(
                    new ApiResponse(
                        409,
                        "User already exists !, try some different email address !"
                    )
                );
        }
        ...
        const data = await updatePropertiesOfUser(
            "PUT",
            userRecievedFromReq.kindeAuthId,
            "profile_complete",
            "true"
        );
        if (!data) {
            throw new ApiError(500, "Failed to update user properties in external service");
        }
        ...
        if (!user) {
            throw new ApiError(400,"Error creating user in mongodb!")
        }
