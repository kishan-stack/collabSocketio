...
import { config } from "./src/auth/kindeAuthSetup.js";
...
app.post("/register", async (req, res) => {
    ...
    const normalizedCollegeName = collegeName
    ...
    try {
        ...
        for (const [sectionName, sectionData] of Object.entries(sections)) {
            if (!Array.isArray(sectionData)) {
                return res
                    .status(400)
                    .json({ error: `${sectionName} must be an array.` });
            }
        }
...
RETURN u, matchingSkills, normalizedScore,matchedSkills;`;
        ...
        const responseData = userList.records.map((record) => ({
            ...
            matchingSkills: neo4j.integer.toNumber(
...
app.get("/user-info", protectRoute, (req, res) => {
    const user = req.user; // User information from the JWT
    console.log(user);
});
...
app.get("/recommendations", async (req, res) => {
    ...
    if (!email) {
        return res
            .status(400)
            .json({ error: "Email is required to get recommendations" });
    }
...
app.get("/testingKindeAuth", (req, res) => {
